...
var _getDependencyConfig = _interopRequireDefault(
  require('./getDependencyConfig')
);

var _getProjectDependencies = _interopRequireDefault(
  require('./getProjectDependencies')
);
...
var _getPlatforms = _interopRequireDefault(require('../core/getPlatforms'));
  ...
  Object.keys(platforms || {}).forEach(platform => {
    ...
    if (!isInstalled) {
      _logger.default.info(
        `Platform '${platform}' module ${packageName} is not installed`
      );

      return;
    }

    _logger.default.info(`Unlinking ${packageName} ${platform} dependency`);

    linkConfig.unregister(
      packageName, // $FlowFixMe: We check for existence on line 38
...
function unlink(args, ctx) {
  ...
  const allDependencies = (0, _getProjectDependencies.default)().map(
    dependency => (0, _getDependencyConfig.default)(ctx, platforms, dependency)
  );
  ...
  const tasks = [
    () =>
      (0, _promisify.default)(
        dependency.commands.preunlink || _commandStub.default
      ),
    () =>
      unlinkDependency(
        platforms,
    ...
    () =>
      (0, _promisify.default)(
        dependency.commands.postunlink || _commandStub.default
      )
  ];
  return (0, _promiseWaterfall.default)(tasks)
    .then(() => {
      // @todo move all these to `tasks` array, just like in
      // link
      const assets = (0, _lodash().difference)(
        dependency.assets,
        (0, _lodash().flatten)(allDependencies, d => d.assets)
      );
